...
public class RDParser extends Lexer {
    ...
    public Exp callExpr(COMPILATION_CONTEXT ctx) {
        currentToken = getToken();
        return Expr(ctx);

    }

    private Exp Expr(COMPILATION_CONTEXT ctx) {
        TOKEN l_token;
        Exp returnValue = Term(ctx);
    ...
    private Exp Term(COMPILATION_CONTEXT ctx) {
        TOKEN l_token;
    ...
    private Exp Factor(COMPILATION_CONTEXT ctx) throws Exception {
        TOKEN l_token;
        Exp returnValue = null;
    ...
    private Stmt ParseAssignmentStatement(COMPILATION_CONTEXT ctx) throws Exception {
        ...
        //type check
        if (exp.TypeCheck(ctx) != symbol.Type) {
