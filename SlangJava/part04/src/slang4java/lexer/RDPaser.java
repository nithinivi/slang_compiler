...
import slang4java.statements.PrintLineStatement;
import slang4java.statements.PrintStatement;
...
public class RDPaser extends Lexer {
    ...
    private Stmt Statement() {
        ...
        switch (currentToken) {
            case TOK_PRINT -> {
                try {
                    returnValue = ParsePrintStatment();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                getNext();
            ...
            case TOK_PRINTLN -> {
                try {
                    returnValue = ParsePrintLnStatment();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                getNext();
    ...
    private Stmt ParsePrintLnStatment() throws Exception {
        getNext();
        Exp a = Expr();
    ...
    private Stmt ParsePrintStatment() throws Exception {
        getNext();
        Exp a = Expr();
        if (currentToken != TOKEN.TOK_SEMI) {
